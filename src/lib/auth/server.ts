...
// Auth and its database connection are initialized inside a request.
...
function createAuth() {
  ...
  const env = (key: string): string | undefined => {
    const raw = (workerEnv as unknown as Record<string, unknown>)[key] ?? process.env[key];
  ...
  const federatedAuthConfigured = !authDisabled && Boolean(grokClientId && grokClientSecret);
  ...
  const grokOAuthPlugin = federatedAuthConfigured
    ? genericOAuth({
  ...
  return betterAuth({
    ...
    secret:
      ...
      (databaseUrl
        ? (() => {
            throw new Error("BETTER_AUTH_SECRET is required with a production database");
          })()
        : previewAuthSecret()),
    database,
