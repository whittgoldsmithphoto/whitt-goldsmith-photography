...
] as (OwnerCatalogPhoto & { updatedAt?: string })[];
...
test("sorts newest updated safely when dates are missing", () => {
  const input = photos.map((photo) => photo.id === "a" ? { ...photo, updatedAt: undefined } : photo);
